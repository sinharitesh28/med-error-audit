...
function addOpdDrug() {
    ...
    const rowHtml = `
        <div class="opd-drug-row" data-drug-id="${drugCount}">
            ...
            <button type="button" class="btn-tag-error" onclick="toggleErrorPanel(this, ${drugCount})">âš ï¸ Tag Error</button>
    ...
    document.querySelectorAll('.opd-drug-row').forEach(async (rowEl) => {
        ...
        if (panel && panel.style.display === 'block') {
            ...
            if(category && subCategory) {
                const fileInput = panel.querySelector('.e-image-file');
                let driveUrl = null;
                if(fileInput.files.length > 0) {
                    fileInput.parentElement.insertAdjacentHTML('beforeend', '<span class="uploading-txt" style="color:var(--secondary); font-size:12px;">Uploading...</span>');
                    driveUrl = await uploadToDrive(fileInput.files[0], category);
                    const upTxt = panel.querySelector('.uploading-txt');
